...
class ColorButton @JvmOverloads constructor(
    ...
    init {
        ...
        rippleColor = if (rippleAlpha != null){
            getCustomColorString(attrs.getInt(R.styleable.ColorButton_colorButtonRippleColor, 0), currentTheme.accentColor)!!.transparency(rippleAlpha!!)
        } else {
            getCustomColorString(attrs.getInt(R.styleable.ColorButton_colorButtonRippleColor, 0), currentTheme.accentColor)!!
        }
        
        val colorText = getCustomColorString(attrs.getInt(R.styleable.ColorButton_colorButtonTextColor, 0))
        
        colorOfText = when (style) {
            1, 2 -> colorText ?: currentTheme.buttonTextColor
            3, 4 -> colorText ?: currentTheme.accentColor
